...
public class Ran_DbHelper extends SQLiteOpenHelper{
	...
	public Ran_DbHelper(Context context) {
		super(context, DATABASE_NAME, null, DATABASE_VERSION);
	}

	@Override
	public void onCreate(SQLiteDatabase db) {
		//gets called when it gets started. If you want onCreate() to run you need to use adb to delete the SQLite database file.
	...
	public boolean createEntry(String presetName, ArrayList<String> playersArrList) throws SQLException{
		...
		for(String name: playersArrList){ //put all players names in 1 string
			playerNames.append(name).append("~"); //delimiter
		}
		...
		try{
			...
			cv.put(KEY_PLAYERS, playerNames.toString());
	...
	public void updatePreset(String presetName, ArrayList<Ran_UpdateObj> updateList){
		String[] columns = new String[]{KEY_PLAYERS};
		StringBuilder playerNamesWDelimiter = new StringBuilder();
		ContentValues cv = new ContentValues();

		ourDatabase.beginTransaction(); //need to start a transaction for multiple sql statements for faster speeds
		try (Cursor cursor = ourDatabase.query(DATABASE_TABLE, columns, KEY_PRESET_NAME + " = '" + presetName + "'", null, null, null, null)) {
			if (cursor.moveToNext()) {
				playerNamesWDelimiter = new StringBuilder(cursor.getString(0));
			}

			for (Ran_UpdateObj up : updateList) { //alter the string
				if (up.getAction().equals("add")) {
					playerNamesWDelimiter.append(up.getPlayerName()).append("~");
				} else if (up.getAction().equals("delete")) {
					playerNamesWDelimiter = new StringBuilder(playerNamesWDelimiter.toString().replaceFirst(up.getPlayerName() + "~", ""));
				}
			}
			cv.put(KEY_PLAYERS, playerNamesWDelimiter.toString());    //put string back in db
